...
$query = mysqli_query($db_conx, $sql);
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
	{
		...
		echo '<div id="container">
				<div id="userphotos"><a href="uploads/'. $id .'"  title="'.cleanString($id).'" class="thickbox">
					<img onclick="photoShowcase('.$row[filename].')" src="user/'.$u.'/'.$row[filename].'"width="100" height="100" alt="pic" /></a></div>	
...
<div id="header">Infinite Scroll</div>   
  <div id="container">
	<?php echo $userphotos; ?>
    <p><?php echo $count; ?> Images Displayed | <a href="#header">top</a></p>
    <br />
    <hr />
  </div>
